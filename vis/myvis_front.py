...
def render_headless(root_data_dir, vis_output_dir, json_path, mesh_color, view_rot, view_name):
    ...
    pos_data, rot_data,removed_idx ,total_steps = getTransform(json_path)
    ...
    for i, base_mesh in enumerate(base_meshes):
        if base_mesh is None or (i in removed_idx):
            scene_meshes.append(None) # Keep index alignment
            continue
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Headless rendering for a single view.")
    ...
    with Display(visible=0, size=(1920, 1080)):
        ...
        front_view_rotation = (-np.pi / 2, np.pi, 0)
