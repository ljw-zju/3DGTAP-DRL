...
def render_headless(root_data_dir, vis_output_dir, json_path, mesh_color, view_rot, view_name, exclude_indices):
    ...
    Args:
        ...
        view_name (str): Name of the view for the output subdirectory (e.g., "left").
    ...
    for i, base_mesh in enumerate(base_meshes):
        # Exclude specified teeth
        if base_mesh is None or i in exclude_indices or i in remove_idx:
            scene_meshes.append(None) # Append None to maintain index alignment
            continue
            
        mesh_copy = copy.deepcopy(base_mesh)
        q = np.array(rot_data[0][i], dtype=np.float64) # Assume format (x, y, z, w)
        q_o3d = np.array([ q[0], q[1], q[2],q[3]])     # Convert to (w, x, y, z) for Open3D

        R = mesh_copy.get_rotation_matrix_from_quaternion(q_o3d)
        mesh_copy.rotate(R, center=(0, 0, 0))
        mesh_copy.translate(pos_data[0][i])
        
        # Apply the fixed view rotation
    ...
    for g_idx in range(total_steps):
        ...
        for i, base_mesh in enumerate(base_meshes):
            # Skip excluded or non-existent teeth
            if base_mesh is None or i in exclude_indices or i in remove_idx:
                continue
            ...
            # Get transformation for the current frame
            ...
            # Update the vertices and normals of the mesh in the scene
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Headless rendering for the 'left' view.")
    parser.add_argument("--target_sample_name", type=str, required=True, help="Name of the sample (e.g., C0100xxxxxx).")
    parser.add_argument("--json_root", type=str, required=True, help="Root directory of the JSON transformation files.")
    parser.add_argument("--model_name", type=str, required=True, help="Name of the model generating the results.")
    parser.add_argument("--is_gt", type=lambda x: (str(x).lower() == 'true'), required=True, help="Is this ground truth data? (true/false)")
    ...
    with Display(visible=0, size=(1920, 1080)):
        ...
        # Define output directory and input JSON path
        if args.is_gt:
        ...
        # Define the rotation for the LEFT view
        left_view_rotation = (np.pi/2, 0, np.pi/2)
        
        # Define the indices of teeth to be EXCLUDED from rendering
        exclude_range = [i for i in range(9, 14)] + [i for i in range(23, 28)]
