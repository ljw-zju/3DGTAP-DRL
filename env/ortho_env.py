...
class OrthoEnv(gym.Env):
    def __init__(self,first_step,seq_state,convex_hull=None,alpha_angle=300,alpha_trans=100,
                 collision_tolerance=0.30,completion_prize=100,gamma=0.95,
                 teeth_ids=None,collision_punishment=1,n_step=3,max_step=200,thre_completion=0.3,shape_id=None,alpha_smooth=200,
                 stage2=False):
        ...
        max_diff=np.load('data/max_movement.npy').astype(dtype=np.float32)
    ...
    def step(self,action,is_load_expert=False,global_step=0):
        ...
        num_punishment=self.calculate_num_punishment(action,global_step)
        ...
        return np.concatenate((self.state,self.last_action),axis=1),reward,done,False,reward_dict,self.collision_record,truncated


    def calculate_num_punishment(self,action,global_step=None):
        return 0
    
    def calculate_smooth_puhishmnet(self,action,last_action):
        tran_punishmet=np.sum(np.abs(action[:,:3]-last_action[:,:3]))
        return -(tran_punishmet*self.alpha_smooth)/(self.num_teeth)
    ...
    def calculate_rotation_reward(self,current_rotations,last_rotations,k=160.0):
    ...
    def calculate_collision_penalty(self,state):
        ...
        for i in range(1,len(self.bvh_objects)):
            ...
            if result.is_collision:
                if result.contacts and self.stage2:
                    ...
                    if penetration_depth>0.3:
                        ...
                        reward-=60
                        continue
                    if penetration_depth>0.25:
                        reward-=40
                        continue
                    if penetration_depth>0.2:
                        reward-=20
                        continue
                    if penetration_depth>0.15:
                        reward-=5
                else:
                    if penetration_depth>0.3:
                        reward-=10
    ...
    def default_action(self,):
        ...
        for step in range(self.n_step):
            ...
            if T+step+1 in self.collision_dict:
                ...
                collision_punishment=self.calculate_collision_penalty(self.seq_state[T+step+1])
   
                self.collision_dict[T+step+1]=collision_punishment

            cur_action=self.seq_state[T+step+1,:,:9]-self.seq_state[T+step,:,:9]
            num_punishment=self.calculate_num_punishment(cur_action)
            if T+step>0:
                ...
                smooth_punishment=0
            cur_reward=reward_trans+reward_angle+collision_punishment+smooth_punishment+num_punishment
